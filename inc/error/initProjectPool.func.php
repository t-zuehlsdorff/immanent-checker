...
/**
  * Initialize the project-error pool with its validator.
  *
  * A project error has the following structure:
  *
  ...
  *
  * Project errors describe violations that belong to the filtered project as a
  * whole. They are not tied to one specific directory or file. If a violation
  * belongs to a concrete directory or file, the corresponding directory or file
  * error API should be used instead.
  *
  ...
  *
  * The validator protects the public error API from storing malformed project
  * errors, which is important because checks can be implemented externally.
  **/
function initProjectPool(): void {

  $cloProjectErrorValidator = function(array $arrData): bool {
